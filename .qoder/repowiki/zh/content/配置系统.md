...
## 更新摘要
**变更内容**
- 配置系统重构：从JavaScript配置文件迁移到JSON配置系统
- 新增activities.json配置文件：支持多游戏类型配置和动态加载
- 角色图标配置更新：从单个图片URL改为charIcons数组
- 动态加载机制：通过fetch API实现运行时配置加载
- 配置验证规则更新：支持JSON格式验证和字段完整性检查
...
**更新** 本版本反映了配置系统的重大架构变更：从JavaScript配置文件迁移到JSON配置系统，新增动态加载机制，支持多游戏类型配置和角色图标数组化。
...
## 核心组件
- 配置文件activities.json：重构为多游戏类型JSON配置结构，包含gameTypes游戏类型列表和gameData各游戏类型的活动配置，支持动态加载和精确的开始/结束时间格式。
- 视图组件：
  - GameCalendar.vue：支持游戏类型切换，聚合周表头、今日指示器与活动条，负责动态加载配置与日期工具。
  - ActivityBar.vue：渲染单个活动条，依据活动类型应用样式，依据精确时间计算宽度与偏移，支持角色图标数组渲染。
...
- 配置层：activities.json定义游戏类型和各游戏的活动配置，作为唯一事实来源，通过fetch API动态加载。
...
## 结论
通过将活动配置从JavaScript文件迁移到JSON配置系统，并采用动态加载机制，系统实现了更高性能和更强灵活性的活动管理能力。开发者可以为不同游戏维护独立的日历和活动配置，支持精确到小时的时间控制。JSON格式的配置文件便于版本控制和团队协作，动态加载机制提供了更好的用户体验。这种设计使得配置系统更加贴近实际业务需求，同时保持了良好的可维护性和扩展性。建议在团队内建立配置校验与变更流程，确保配置质量和一致性。
...
- 参考路径
  - JSON配置文件：[activities.json:1-246](file://public/config/activities.json#L1-L246)
  - 活动条渲染：[ActivityBar.vue:1-316](file://src/components/ActivityBar.vue#L1-L316)
