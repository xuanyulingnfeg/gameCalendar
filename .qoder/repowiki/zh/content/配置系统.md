...
**更新** 本版本反映了配置系统的重大架构变更：从JavaScript配置文件迁移到JSON配置系统，新增动态加载机制，支持多游戏类型配置和角色图标数组化。新增绿色活动类型支持，为特殊活动提供更丰富的视觉标识。
...
## 核心组件
- 配置文件activities.json：重构为多游戏类型JSON配置结构，包含gameTypes游戏类型列表和gameData各游戏类型的活动配置，支持动态加载和精确的开始/结束时间格式。现已支持绿色活动类型的配置。
- 视图组件：
...
**更新** 新增绿色活动类型，提供独特的视觉标识，适用于特殊活动如前瞻直播等。
...
**更新** 新增绿色活动类型使用指南，适用于需要特殊视觉标识的活动场景。
...
**更新** 新增绿色活动类型的业务逻辑调整指南，支持特殊活动的灵活配置。
...
**更新** 新增绿色活动类型的配置验证规则，确保新活动类型的有效性。
...
**更新** 新增绿色活动样式故障排查指南，帮助解决绿色活动类型的显示问题。
...
**更新** 新增绿色活动类型支持，进一步丰富了活动视觉标识体系，为特殊活动提供了独特的视觉表现。系统现已支持red、orange、gray、green四种活动类型，满足多样化的业务需求。建议在团队内建立配置校验与变更流程，确保配置质量和一致性。
...
- 参考路径
  - JSON配置文件：[activities.json:1-256](file://public/config/activities.json#L1-L256)
  - 活动条渲染：[ActivityBar.vue:1-321](file://src/components/ActivityBar.vue#L1-L321)
